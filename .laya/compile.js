...
gulp.task("compile", prevTasks, function () {
    ...
    // 注意一：这里做了修改，如果不进行修改，无法进行编译 By Jingkun
	let plgnTsTsConfig = "tsconfig.json";
	let plgnTsInclude  = "**/*.ts";
	let plgnTsOverride = {compilerOptions: { removeComments: true} };
	if (useCMDNode) {
		plgnTsTsConfig = workSpaceDir + "/" + plgnTsTsConfig;
		plgnTsInclude  = workSpaceDir + "/" + plgnTsInclude;
	}
	...
	return rollup.rollup({
		...
		plugins: [
			typescript({
				...
				tsconfigOverride: plgnTsOverride,
				tsconfig: plgnTsTsConfig,
				include: plgnTsInclude
			}),
			glsl({
