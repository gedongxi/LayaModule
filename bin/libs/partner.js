...
// console.log("##########partner_ext" + window.PARTNER_EXT);
// console.log("##########window.loadLib" + window.loadLib);
if (window.PARTNER_EXT && window.loadLib) {
  loadLib(window.PARTNER_EXT);
}

Laya.URL.calcRealUrl = function (url, isFull) {
  if (!url) {
      return url;
  }
  isFull = isFull === true;
  ...
  if (window._DYMappings) {
      // 保存已经找到过的映射
      if (!window.__DYRelMappings) {
          window.__DYRelMappings = {};
      }

      // 如果找到了之前存储的就直接返回
      var realUrl = window.__DYRelMappings[url];
      if (realUrl) {
          // 特殊情况的url
          if (realUrl.url) {
              return realUrl.url;
          ...
          if (isFull) {
              return Partner.CDN_HOST + realUrl;
          }
          return realUrl;
      }

      var strBuf = new DyGame.MixStringBuffer();
      strBuf.setOri(url);
      var pathKey = strBuf.toString();
      realUrl = window._DYMappings[pathKey];
      if (!!realUrl) {
          delete window._DYMappings[pathKey];

          if (realUrl.startsWith("sub2/")) {
              window.__DYRelMappings[url] = { url: realUrl, sub: true };
              return realUrl;
          } else {
              window.__DYRelMappings[url] = realUrl;
              if (isFull) {
                  return Partner.CDN_HOST + realUrl;
              }
              return realUrl;
