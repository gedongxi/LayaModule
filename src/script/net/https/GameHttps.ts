...
export default class GameHttps {
  ...
  // 打点2 get 请求
  public sendPonit2HttpsGet(path, data, handler, extraUrl = null, extraData1 = 0, extraData2 = 0) {
    if (Partner.PARTNER_NAME !== "Dev") {
      ...
      if (data) {
        requestURL += Partner.PARTNER_ID + "/";
        requestURL += DataPlayer.INSTANCE.PlayerId + "/";
        requestURL += data;
      }

      requestURL += "/" + extraData1;
      requestURL += "/" + extraData2;
  ...
  // 打点3 新的广告打点 post不用了
  public sendAdPonitHttpsGet(data, handler) {
    if (Partner.PARTNER_NAME !== "Dev") {

      // 拼接地址
      let requestURL = this.getAdurl;

      if (data) {
        requestURL += Partner.PARTNER_ID + "/";
        requestURL += DataPlayer.INSTANCE.PlayerId + "/";
        requestURL += data.entry + "/";
        requestURL += data.switch + "/";
        requestURL += data.location + "/";
        requestURL += data.ad_id + "/";
        requestURL += data.type + "/";
        requestURL += data.result;
      }
      this.httpsGet(requestURL, handler);
    }
  }
  ...
  // 互推打点
  public sendMutualPushHttpsGet(data, handler) {
    if (Partner.PARTNER_NAME !== "Dev") {

      // 拼接地址
      let requestURL = this.getmutualUrl;

      if (data) {
        requestURL += Partner.PARTNER_ID + "/";
        requestURL += DataPlayer.INSTANCE.PlayerId + "/";
        requestURL += DataPlayer.INSTANCE.RegisterTime + "/";
        requestURL += data.panelName + "/";
        requestURL += data.appId + "/";
        requestURL += data.type + "/";
        requestURL += data.result + "/";
        // 两个-1是预留参数  留着以后用
        requestURL += "-1/-1";
