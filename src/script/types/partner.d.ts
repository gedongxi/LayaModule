declare module Partner {
  ...
  let SERVER_GROUP: string;
  let SERVER_ID: number;
  const AUDIT_SERVER_GROUP: string;
  const PARTNER_NAME: string;
  const PARTNER_ID: number;
  const CHANNEL: number;
  const CDN_HOST: string;
  const HEAD_IMG_HOST: string;
  const SHARE_IMG_HOST: string;
  const userInfo: any;
  const SHOW_GM: boolean;             // 显示gm开关
  const DEBUG_INFO: boolean;          // 显示日志信息
  const VS_WIN: boolean;              // 战斗一键胜利
  const SHOW_SERVER_LIST: boolean;
  let OPPERATION_JSON_URL: string; 
  let OPPERATION_JSON: any;
  let AUDIT_VERSION: string;
  let OTHER_GAME_JSON_URL: string; 
  let OTHER_GAME_JSON: any;
  
  // 解压配置文件
  function unzipConfig(fileBin: any): any;
