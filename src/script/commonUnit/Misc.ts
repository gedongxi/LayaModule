...
import { DiffTime } from "../scene/base/BaseDefine";
...
export default class Misc {
  ...
  //   const count = endValue - beginValue + 1;
  //   if (count < m) {
  //     return;
  //   }
  ...
  //   for (let i = 0; i < count; i++) {
  //     const r = Misc.getRandomInt(i, count - 1);
  ...
  // 计算挑战记录时间 00-00 00:00
  public static calPkRecordTimeShow(pNumTime: number): string {
    const time = new Date(pNumTime * 1000);
    const m = time.getMonth() + 1;
  ...
  // 设置透明度
  public static setOpacity(pSprite: any, pNumOpacity: number) {
    let tMaterial = null;
    const tSprite3D = pSprite;
    console.log();
    if (tSprite3D instanceof Laya.MeshSprite3D) {
      tMaterial = (tSprite3D as Laya.MeshSprite3D).meshRenderer.material;
    } else if (tSprite3D instanceof Laya.SkinnedMeshSprite3D) {
      tMaterial = (tSprite3D as Laya.SkinnedMeshSprite3D).skinnedMeshRenderer.material;
    } else {
      tMaterial = null;
    }
    if (tMaterial) {
        if (tMaterial instanceof Laya.UnlitMaterial) {
            tMaterial.albedoColorA = pNumOpacity;
            tMaterial.renderMode = pNumOpacity === 1 ? 0 : 2;
            tMaterial.depthWrite = true;
        } else if (tMaterial instanceof Laya.BlinnPhongMaterial) {
            tMaterial.albedoColorA = pNumOpacity;
            tMaterial.renderMode = pNumOpacity === 1 ? 0 : 2;
        } else if (tMaterial instanceof Laya.EffectMaterial) {
            const tInitOpacity = tMaterial.colorA;
            tMaterial.color.w = pNumOpacity * tInitOpacity;
        }
    }
  }
